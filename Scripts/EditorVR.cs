		...
		void UpdateMenuVisibilityNearWorkspaces()
		{
			ForEachRayOrigin((proxy, pair, device, deviceData) =>
			{
				...
				for (int i = 0; i < m_UpdateVisibilityMenus.Count; i++)
				{
					...
					float maxComponent; // Because menus can change size, store the maximum size to avoid ping ponging visibility
					if (!menuSizes.TryGetValue(menu, out maxComponent))
					{
						maxComponent = menuBoundsSize.AveragedComponents();
					...
					for (int j = 0; j < m_Workspaces.Count; j++)
					{
						...
						if (flags == 0)
						{
							...
							outerBounds.extents -= Vector3.one * maxComponent * extentsReduction;
						}
