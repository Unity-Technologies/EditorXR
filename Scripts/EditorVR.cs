...
public class EditorVR : MonoBehaviour
{
    ...
    private void CreateDeviceDataForInputDevices()
    {
        foreach (var device in InputSystem.devices)
        {
            var deviceData = new DeviceData
            {
                ...
                menuInput = (Menu)CreateActionMapInput(m_MenuActionMap, device) // TODO: previous implementation, remove, menu is now only left hand
            };
    ...
    private void SpawnMainMenu(Type type)
    {
        ...
        if (m_MainMenu == null)
        {
            var mainMenuGameObject = U.Object.InstantiateAndSetActive(m_MainMenuPrefab.gameObject, transform);
            m_MainMenu = mainMenuGameObject.GetComponent<IMainMenu>();
            m_MainMenu.menuTools = m_AllTools.ToList();
            m_MainMenu.selectTool = SelectTool;
            m_MainMenu.menuActionInput = m_MenuActionInput;
            m_MainMenu.eventCamera = m_EventCamera;
        }

        foreach (var kvp in m_DeviceData)
        {
            var device = kvp.Key;
            if (m_DeviceData[device].mainMenu == null)
            {
                if (m_MainMenu != null)
                {
                    m_DeviceData[device].mainMenu = m_MainMenu;
                    ConnectInterfaces(m_MainMenu, device);
                    UpdatePlayerHandleMaps();
                }
            }
        }
    ...
    private void ConnectInterfaces(object obj, InputDevice device = null)
    {
        ...
        var iMainMenu = obj as IMainMenu;
        if (iMainMenu != null)
        {
            foreach (var proxy in m_AllProxies)
            {
                ...
                if (proxy.menuOrigin != null)
                {
                    iMainMenu.menuOrigin = proxy.menuOrigin;
                    iMainMenu.menuInputOrigin = proxy.menuInputOrigin;
                    break;
                }
            }
        }
