...
namespace UnityEditor.Experimental.EditorVR.Tools
{
	internal sealed class BlinkLocomotionTool : MonoBehaviour, ITool, ILocomotor, ICustomRay, IUsesRayOrigin, ICustomActionMap
	{
		...
		private IEnumerator MoveTowardTarget(Vector3 targetPosition)
		{
			...
			targetPosition = new Vector3(targetPosition.x + (viewerPivot.position.x - CameraUtils.GetMainCamera().transform.position.x), viewerPivot.position.y, targetPosition.z + (viewerPivot.position.z - CameraUtils.GetMainCamera().transform.position.z));
