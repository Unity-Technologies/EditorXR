...
{
	[UnityEngine.VR.Tools.VRMenuItem("Select", "Transform", "Select items in the scene")]
	public class SelectionTool : MonoBehaviour, ITool, IRay, IRaycaster, ICustomActionMap, IHighlight, IUsesActions, IMenuOrigins
	{
		...
		public Action<HashSet<GameObject>> displayRadialMenu { private get; set; }
			...
			if (m_SelectionInput.select.wasJustPressed) 
			{
				...
				else
				{
					...
					else
					{
						...
						Selection.activeGameObject = m_HoverGameObject;
						s_SelectedObjects.Add(m_HoverGameObject);
						m_RadialMenu.Show(actions); // Show the radial menu if there are any objects in the set, hide it otherwise.
					}
				}
				Selection.objects = s_SelectedObjects.ToArray();
			}
