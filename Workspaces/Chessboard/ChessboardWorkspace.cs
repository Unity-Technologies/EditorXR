...
public class ChessboardWorkspace : Workspace
{
	...
	void Update()
	{
		...
		m_GridMaterial.mainTextureScale = new Vector2(contentBounds.size.x * m_MiniWorld.clipBox.transform.localScale.x, contentBounds.size.z * m_MiniWorld.clipBox.transform.localScale.x) * kGridScale;
		m_GridMaterial.mainTextureOffset = new Vector2(m_GridMaterial.mainTextureScale.x % 2, m_GridMaterial.mainTextureScale.y % 2) * -0.5f + Vector2.one * 0.5f;
	}
