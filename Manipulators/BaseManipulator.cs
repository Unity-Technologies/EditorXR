...
namespace UnityEngine.VR.Manipulators
{
	public class BaseManipulator : MonoBehaviour, IManipulator
	{
		...
		void OnCameraPreRender(Camera camera)
		{
			...
			var manipulatorPosition = camera.worldToCameraMatrix.MultiplyPoint3x4(transform.position);
			var cameraPosition = camera.worldToCameraMatrix.MultiplyPoint3x4(camera.transform.position);
			var delta = camera.cameraToWorldMatrix.MultiplyPoint3x4(cameraPosition - manipulatorPosition);
